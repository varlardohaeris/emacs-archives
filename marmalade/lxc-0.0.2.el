...
(defun lxc-destroy (id)
  "Destroy the specified LXC container.

This deletes the container information, so it's removed from the
list."
  (interactive (lxc/get-id))
  (shell-command (format "sudo lxc-destroy -n %s" id))
  (when (called-interactively-p 'any)
    (tabulated-list-print)))
...
(define-derived-mode
    ...
    (define-key lxc-list-table-mode-map (kbd "D") 'lxc-destroy)
