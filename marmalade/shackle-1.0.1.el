...
window."
  ...
  (let* ((ignore-window-parameters t)
    ...
    (when (and (plist-get plist :select) (window-live-p window))
      (select-window window t))
    (when (and (plist-get plist :fit) (window-live-p window))
      (let ((size (plist-get plist :size)))
        (if size
            (cond
             ((window-combined-p window)
              (when (< size 1)
                (setq size (round (* size (window-size (frame-root-window))))))
              (fit-window-to-buffer window size))
             ((window-combined-p window t)
              (when (< size 1)
                (setq size (round (* size (window-size (frame-root-window) t)))))
              (fit-window-to-buffer window nil nil size)))
          (when (or (window-combined-p window)
                    (window-combined-p window t))
