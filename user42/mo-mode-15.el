...
;; Version 13 - compilation-find-file args by number, so not depend on names
;; Version 14 - cl macros only when needed
;; Version 15 - no internal macros after byte compile
...
;; Explicit dependency on advice.el since `mo-mode-unload-function' needs
...
(eval-when-compile
  (defmacro mo-mode--with-errorfile (&rest body)
    "An internal part of mo-mode.el.
...
;; quieten byte compiler for emacs21,xemacs21 which don't have this var
(defvar po-content-type-charset-alist)
...
`mm-charset-to-coding-system'."
  ...
  (or
   ...
   (and (or (ignore-errors (require 'po))
            (ignore-errors (require 'po-compat)))
...
;; LocalWords: el po mo gmo nl langinfo msgfmt msgunfmt msgstr unibyte multibyte multibyteness gettext Gettext builtin ok lookups libc iconv charset
