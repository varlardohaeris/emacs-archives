...
Return nil if ok, so never superseeked, enough time passed since
last seek, or user asks to continue.
...
time and warns if too soon.

The last time is in file ~/.emacs.d/oeis-superseeker-time.el so
is preserved across sessions.  If the recorded time is somehow
wrong you can always say \"yes\" to send anyway, or delete that
file.  ~/.emacs.d is `user-emacs-directory'.
...
messages since the reply shows what was searched anyway.  In Gnus
that can be done with some code in `gnus-message-archive-group'
to archive to nil.
...
yet in the buffer.  There's a dynamic binding of the `to'
variable from `compose-mail', but that's not a documented feature
and if it went lexical would be unavailable."
