...
;; Copyright 2006, 2009, 2010, 2011, 2013, 2014, 2016, 2019 Kevin Ryde
...
;; `M-x aumix' runs the "aumix" mixer program (sound volume adjuster) in a
;; buffer.  This is the same as in a tty, but within Emacs.  See the
;; docstrings of `aumix' and `aumix-mode' for details.
...
;; Version 8 - allow for non-existent default-directory
...
The \"-C\" colour file option can choose colours other than the
default \"ansi\" style.  An absolute filename is recommended so
as not to accidentally strike something in the current directory.
Eg. the aumix supplied vt100 file is black and white

    (setq aumix-mode-switches '(\"-C\" \"/usr/share/aumix/vt100\"))"
...
"
  ...
  ;; If `default-directory' is a non-existent directory for whatever reason
  ;; (inherited from a buffer visiting a new file, directory removed after
  ;; visiting, etc) then the start-process (in term-exec) will fail.
  ;; The directory shouldn't matter much to aumix, so if default-directory
  ;; doesn't exist then go to either the home directory or root directory.
  ;;
