...
;; size macros for TeX and LaTeX.  For example \bigl( increase to \Bigl(.
;;
;; Similar can be found in `YaTeX-change-parentheses', but it has read-char
;; for desired new size whereas tex-increase just goes a size up or down.
;; tex-increase has font sizes and some skips increase or decrease too
;; (see `tex-increase-lists' for all).
...
(defconst tex-increase-regexp
  ...
  (eval-when-compile
    (concat "\\("
            (regexp-opt (apply 'append (mapcar (lambda (l) (remq nil l))
                                               tex-increase-lists))
                        t)
...
describes.  Plain TeX \\=\\> still works.
...
URL `http://user42.tuxfamily.org/tex-increase/index.html'"
  ...
  (save-restriction
    ...
    (while (/= 0 arg)
      (or (looking-at tex-increase-regexp)       ;; prefer after point
          (looking-at tex-increase-regexp-paren) ;; prefer after point
          (thing-at-point-looking-at tex-increase-regexp)
      ...
      (if (match-beginning 1)
          (let (pos lst)
            (dolist (l tex-increase-lists)
              (unless pos
                (setq pos (tex-increase-position-in-list (match-string 1) l))
                (setq lst l)))
            (when pos
              (setq pos (+ pos (if (> arg 0) 1 -1)))
              (when (or (< pos 0)
                        (>= pos (length lst)))
                (error "Nothing more for \\%s" (match-string 1)))
              ...
              (let ((rep (nth pos lst)))
                ...
                (tex-increase-replace (match-beginning 1) (match-end 1)
                                      (or rep "")))))
